  ...
  module Models
    module QueryOverride
      ...
      module ExtendedQueries
        ...
        # TODO : remove hide checkbox if not enable on project, already done UPSTREAM
      ...
      #*****************
      # 2/ WatcherValues
      module WatcherValues
        # extend ActiveSupport::Concern

        def self.prepended(base)
          #####################
          # 1/ Instance methods
          watcher_values_instance_methods = [
            :watcher_values,  #  1/ new method  TESTED           OK
          ]

          trace_prefix = "#{' ' * (base.name.length + 27)}  --->  "
          last_postfix = '< (SM::MO::QueryOverride::WatcherValues)'
          ...
          missing_instance_methods = watcher_values_instance_methods.select{|m|
            !smile_instance_methods.include?(m)
          }
        ...
        # Smile specific : if not defined upstream on previous Redmine versions
        def watcher_values
